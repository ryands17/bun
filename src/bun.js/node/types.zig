...
pub const Path = struct {
    ...
    inline fn getPathLenT(comptime T: type, lenSum: usize) usize {
        return @max(lenSum, PathSize(T).DEFAULT);
    }
    ...
    pub fn PathSize(comptime T: type) type {
        return struct {
            const DEFAULT: usize = if (T == u16) PATH_MIN_WIDE else bun.MAX_PATH_BYTES;
            const MAX: usize = if (T == u16) windows.PATH_MAX_WIDE else bun.MAX_PATH_BYTES;
        };
    }
    ...
    pub inline fn formatPosixJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, pathObject: PathParsed(T), comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        return formatPosixJS_T(T, globalObject, pathObject, &buf);
    }

    pub inline fn formatWindowsJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, pathObject: PathParsed(T), comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        return formatWindowsJS_T(T, globalObject, pathObject, &buf);
    }
    ...
    pub fn formatJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, pathObject: PathParsed(T)) JSC.JSValue {
        ...
        const lenSum: usize = 1 +
            (if (dirLen > 0) dirLen else pathObject.root.len) +
            (if (baseLen > 0) baseLen else pathObject.name.len + pathObject.ext.len);
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, lenSum)) {
            size.DEFAULT => if (isWindows) formatWindowsJS_L(T, globalObject, pathObject, size.DEFAULT) else formatPosixJS_L(T, globalObject, pathObject, size.DEFAULT),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                break :brk if (isWindows) formatWindowsJS_T(T, globalObject, pathObject, buf) else formatPosixJS_T(T, globalObject, pathObject, buf);
            },
        };
    }
    ...
    pub inline fn joinPosixJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, paths: []const []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        var buf2: [len]T = undefined;
        return joinPosixJS_T(T, globalObject, paths, &buf, &buf2);
    }

    pub inline fn joinWindowsJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, paths: []const []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        var buf2: [len]T = undefined;
        return joinWindowsJS_T(T, globalObject, paths, &buf, &buf2);
    }
    ...
    pub fn joinJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, paths: []const []const T) JSC.JSValue {
        ...
        var lenSum: usize = if (isWindows) 8 else 0;
        for (paths) |path| lenSum += if (lenSum > 0 and path.len > 0) path.len + 1 else path.len;
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, lenSum)) {
            size.DEFAULT => if (isWindows) joinWindowsJS_L(T, globalObject, paths, size.DEFAULT) else joinPosixJS_L(T, globalObject, paths, size.DEFAULT),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf2);
                break :brk if (isWindows) joinWindowsJS_T(T, globalObject, paths, buf, buf2) else joinPosixJS_T(T, globalObject, paths, buf, buf2);
            },
        };
    }
    ...
    pub inline fn normalizePosixJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, path: []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        return normalizePosixJS_T(T, globalObject, path, &buf);
    }

    pub inline fn normalizeWindowsJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, path: []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        return normalizeWindowsJS_T(T, globalObject, path, &buf);
    }
    ...
    pub fn normalizeJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, path: []const T) JSC.JSValue {
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, path.len)) {
            size.DEFAULT => if (isWindows) normalizeWindowsJS_L(T, globalObject, path, size.DEFAULT) else normalizePosixJS_L(T, globalObject, path, size.DEFAULT),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                break :brk if (isWindows) normalizeWindowsJS_T(T, globalObject, path, buf) else normalizePosixJS_T(T, globalObject, path, buf);
            },
        };
    }
    ...
    pub inline fn relativePosixJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, from: []const T, to: []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        var buf2: [len]T = undefined;
        var buf3: [len]T = undefined;
        return relativePosixJS_T(T, globalObject, from, to, &buf, &buf2, &buf3);
    }

    pub inline fn relativeWindowsJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, from: []const T, to: []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        var buf2: [len]T = undefined;
        var buf3: [len]T = undefined;
        return relativeWindowsJS_T(T, globalObject, from, to, &buf, &buf2, &buf3);
    }
    ...
    pub fn relativeJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, from: []const T, to: []const T) JSC.JSValue {
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, from.len + to.len)) {
            size.DEFAULT => if (isWindows) relativeWindowsJS_L(T, globalObject, from, to, size.DEFAULT) else relativePosixJS_L(T, globalObject, from, to, size.DEFAULT),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                ...
                break :brk if (isWindows)
                    relativeWindowsJS_T(T, globalObject, from, to, buf, buf2, buf3)
                else
                    relativePosixJS_T(T, globalObject, from, to, buf, buf2, buf3);
            },
        };
    }
    ...
    pub fn resolvePosixT(comptime T: type, paths: []const []const T, buf: []T, buf2: []T) MaybeSlice(T) {
        ...
        while (i_i64 > -2 and !resolvedAbsolute) : (i_i64 -= 1) {
            ...
            if (i_i64 >= 0) {
                ...
                var tmpBuf: [PathSize(T).MAX]T = undefined;
    ...
    pub inline fn resolvePosixJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, paths: []const []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        var buf2: [len]T = undefined;
        return resolvePosixJS_T(T, globalObject, paths, &buf, &buf2);
    }

    pub inline fn resolveWindowsJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, paths: []const []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        var buf2: [len]T = undefined;
        return resolveWindowsJS_T(T, globalObject, paths, &buf, &buf2);
    }
    ...
    pub fn resolveJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, paths: []const []const T) JSC.JSValue {
        ...
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, lenSum)) {
            size.DEFAULT => if (isWindows) resolveWindowsJS_L(T, globalObject, paths, size.DEFAULT) else resolvePosixJS_L(T, globalObject, paths, size.DEFAULT),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf2);
                break :brk if (isWindows) resolveWindowsJS_T(T, globalObject, paths, buf, buf2) else resolvePosixJS_T(T, globalObject, paths, buf, buf2);
            },
        };
    }
    ...
    pub inline fn toNamespacedPathWindowsJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, path: []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        var buf2: [len]T = undefined;
    ...
    pub fn toNamespacedPathJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, allocator: std.mem.Allocator, isWindows: bool, path: []const T) JSC.JSValue {
        ...
        const size = comptime PathSize(T);
        return switch (getPathLenT(T, path.len)) {
            size.DEFAULT => toNamespacedPathWindowsJS_L(T, globalObject, path, size.DEFAULT),
            else => |bufLen| brk: {
                const buf = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(T, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf2);
                break :brk toNamespacedPathWindowsJS_T(T, globalObject, path, buf, buf2);
            },
        };
    }
